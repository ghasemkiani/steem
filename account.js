...
class Account extends Obj {
  ...
  get voting_power_now() {
    return Math.min(
      this.voting_power +
        quantity.time().period(this.date_last_vote).u("day").n() * 20,
      100,
    );
  }
  tm_to_full() {
    return quantity
      .time()
      .u("hr")
      .n((100 - this.voting_power_now) * 1.2)
      .u("min");
  }
  date_full() {
  ...
  get vote_value_usd() {
    let half1 = asset
      .usd()
      .steem(this.vote_value / 2)
      .usd()
      .n();
    let half2 = asset
      .usd()
      .sbd(this.vote_value_sbd / 2)
      .usd()
      .n();
    return half1 + half2;
  }
  ...
  async toClaimRewardBalance(dontUpdateData, dontUpdateGlobals) {
    ...
    if (
      ...
      return await sutil.toClaimRewardBalance(
        wif,
        account,
        rewardSteem,
        rewardSbd,
  ...
  async toPowerDown(vests) {
    return await sutil.toPowerDown(
      this.auth.active.key,
      this.username,
      asset.vests(vests).s(),
    );
  }
  ...
  async toGetFollowing(toCallback, startFollowing) {
    return await sutil.toGetFollowing(
      this.username,
      toCallback,
      startFollowing,
    );
  ...
  async toFollow(following) {
    return await sutil.toFollow(
      this.auth.posting.key,
      this.username,
      following,
    );
  }
  async toUnfollow(following) {
    return await sutil.toUnfollow(
      this.auth.posting.key,
      this.username,
      following,
    );
  ...
  async toGetBids(arg) {
    ...
    let date = post
      ? post.date_created || quantity.time().now().u("day").delta(-7).date()
      : quantity.time().now().u("day").delta(-days).date();
    let bids = [];
    let refunds = {};
    let from = this.username;
    await this.toGetHistory({
      onItem(item, items) {
        if (quantity.time().period(item.date, date).n() > 0) {
          ...
          item.op.kind === "transfer" &&
          item.op.from === from &&
          (post
            ? item.op.memo === url
            : String(item.op.memo).startsWith(url_stub))
        ) {
  ...
  async toCreateLimitOrder(
    ...
    result = await sutil.toCreateLimitOrder(
      wif,
      owner,
      orderid,
      amountToSell,
  ...
  async toSell(steem, sbd) {
    ...
    let fillOrKill = true;
    let expiration = cutil.asInteger(
      quantity.time().now().u("min").delta(5).u("sec").n(),
    );
    result = await sutil.toCreateLimitOrder(
      wif,
      owner,
