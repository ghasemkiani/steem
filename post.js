...
class Post extends Obj {
  ...
  async toProcessData(updateGlobals) {
    ...
    if (this.data.active_votes) {
      this.data.active_votes.forEach((active_vote) =>
        this.addVote(active_vote),
      );
    }
    ...
    this.totrshares = this.votes
      .map((vote) => vote.rshares)
      .reduce((a, b) => a + b, 0);
    this.totrwshares = this.votes
      .map((vote) => vote.rwshares)
      .reduce((a, b) => a + b, 0);
    this.totweights = this.votes
    ...
    this.votes.forEach((vote) => {
      vote.wtrs =
        vote.weight == 0 || vote.rwshares == 0
          ? 0
          : vote.weight / this.totweights / (vote.rshares / this.totrshares);
    });
    ...
    this.totrsstm = this.votes
      .map((vote) => vote.rsstm)
      .reduce((a, b) => a + b, 0);
    this.totrwstm = this.votes
      .map((vote) => vote.rwstm)
      .reduce((a, b) => a + b, 0);
  ...
  vote(account, percent, date) {
    ...
    let weight =
      Math.sqrt(this.net_rshares + rshares) - Math.sqrt(this.net_rshares);
