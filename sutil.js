...
class SUtil extends Obj {
  ...
  get msUpdateInterval() {
    if (cutil.isNil(this._msUpdateInterval)) {
      this._msUpdateInterval = !cutil.isNil(
        process.env.CRYPTO_PRICE_UPDATE_INTERVAL,
      )
        ? process.env.CRYPTO_PRICE_UPDATE_INTERVAL
        : 0;
    }
  ...
  async toGetDynamicGlobalProperties() {
    this.dynamicGlobalProperties =
      await this.steem.api.getDynamicGlobalPropertiesAsync();
    return this.dynamicGlobalProperties;
  }
  async toGetCurrentMedianHistoryPrice() {
    this.currentMedianHistoryPrice =
      await this.steem.api.getCurrentMedianHistoryPriceAsync();
    this.price =
      parseFloat(this.currentMedianHistoryPrice.base) /
      parseFloat(this.currentMedianHistoryPrice.quote);
  ...
  async toUpdateGlobals() {
    ...
    let now = Date.now();
    if (
      cutil.isNil(this.lastUpdateTime) ||
      now - this.lastUpdateTime >= this.msUpdateInterval
    ) {
      this.lastUpdateTime = now;
      await this.toGetPrices();
    }
  ...
  vestsToSP(vests) {
    ...
    let totVests = this.assetNum(
      this.dynamicGlobalProperties.total_vesting_shares,
    );
    let totSteem = this.assetNum(
      this.dynamicGlobalProperties.total_vesting_fund_steem,
    );
  ...
  async toTransfer(wif, from, to, amount, memo) {
    return await this.steem.broadcast.transferAsync(
      wif,
      from,
      to,
      amount,
  ...
  async toPowerUp(wif, from, to, amount) {
    return await this.steem.broadcast.transferToVestingAsync(
      wif,
      from,
      to,
      amount,
  ...
  async toPowerDown(wif, account, vestingShares) {
    return await this.steem.broadcast.withdrawVestingAsync(
      wif,
      account,
      vestingShares,
    );
  ...
  async toDelegate(wif, delegator, delegatee, vesting_shares) {
    return await this.steem.broadcast.delegateVestingSharesAsync(
      wif,
      delegator,
      delegatee,
      vesting_shares,
  ...
  async toVote(wif, voter, author, permlink, weight) {
    return await this.steem.broadcast.voteAsync(
      wif,
      voter,
      author,
      permlink,
  ...
  async toCreateLimitOrder(
    ...
    return await this.steem.broadcast.limitOrderCreateAsync(
      wif,
      owner,
      orderid,
      amountToSell,
  ...
  async toCancelLimitOrder(wif, owner, orderid) {
    return await this.steem.broadcast.limitOrderCancelAsync(
      wif,
      owner,
      orderid,
    );
  }
  async toClaimRewardBalance(
    wif,
    account,
    rewardSteem,
    rewardSbd,
    ...
    return await this.steem.broadcast.claimRewardBalanceAsync(
      wif,
      account,
      rewardSteem,
      rewardSbd,
  ...
  async toGetAccountHistory(arg) {
    ...
    let limit = arg.limit;
    loop: while (true) {
      let batch = await this.steem.api.getAccountHistoryAsync(
        username,
        arg.forward ? from + limit : from,
        !arg.forward && from !== -1 && from < limit ? from : limit,
      );
  ...
  async toComment(
    ...
    return await this.steem.broadcast.commentAsync(
      wif,
      parentAuthor,
      parentPermlink,
      author,
  ...
  async toGetFollowers(username, toCallback, startFollower) {
    ...
    let hasNext;
    do {
      array = await this.steem.api
        .getFollowersAsync(following, startFollower, followType, limit)
  ...
  async toGetFollowing(username, toCallback, startFollowing) {
    ...
    do {
      array = await this.steem.api
        .getFollowingAsync(follower, startFollowing, followType, limit)
  ...
  async toCheckFollower(following, follower) {
    ...
    let array = await this.steem.api
      .getFollowersAsync(following, startFollower, followType, limit)
      .map((item) => item.follower);
  ...
  async toFollow(wif, follower, following) {
    ...
    let followType = "blog";
    let result = await this.steem.broadcast.customJsonAsync(
      wif,
      [],
      [follower],
      "follow",
      JSON.stringify(["follow", { follower, following, what: [followType] }]),
    );
  ...
  async toStreamBlocks(arg) {
    ...
    return new Promise((resolve, reject) => {
      let release = this.steem.api.streamBlockNumber(
        arg.irreversible ? "irreversible" : "head",
        async (e, blockNumber) => {
          if (e) {
            release();
            reject(e);
  ...
  async toGetVestsPriceAtBlock(blockNumber) {
    ...
    let price;
    loop: for (let bn = blockNumber; bn > 0; bn--) {
      let block = await this.toGetBlock(bn);
      for (let item of block.transactions) {
        if (
          item.op.kind === "fill_vesting_withdraw" &&
          parseFloat(item.op.withdrawn) > 0
        ) {
      ...
      loop: while (true) {
        ...
        for (let item of block.transactions) {
          if (
            ...
            price =
              parseFloat(item.op.deposited) / parseFloat(item.op.withdrawn);
            break loop;
          }
        }
      }
  ...
  async toGetPostsByTag(arg) {
    ...
    while (contents.length < count) {
      ...
      let batch = await this.steem.api.getDiscussionsByCreatedAsync({
        tag,
        limit,
        start_author,
        start_permlink,
  ...
  async toResteem(wif, account, author, permlink) {
    return await this.steem.broadcast.customJsonAsync(
      wif,
      [],
      [account],
      "follow",
      JSON.stringify(["reblog", { account, author, permlink }]),
    );
  }
  ...
  async toGetAccountCreationFee() {
    ...
    return (
      this.config.STEEM_CREATE_ACCOUNT_WITH_STEEM_MODIFIER *
      this.assetNum(this.chainProperties.account_creation_fee)
    );
  }
